...
export const fetchUserById = async (req, res, next) => {
    try{
        logger.info(`Fetching user with ID: ${req.params.id} by user ${req.user?.id}`);
        ...
        logger.info(`User ${validatedId.id} fetched successfully by user ${req.user?.id}`);
        res.json({
...
export const updateUserData = async (req, res, next) => {
    try{
        logger.info(`Updating user with ID: ${req.params.id} by user ${req.user?.id}`);
        ...
        if (validatedUpdates.role && userRole !== 'admin') {
            logger.warn(`User ${req.user?.id} attempted to update role without admin permission`);
        ...
        if (validatedUpdates.role && userRole === 'admin') updateData.role = validatedUpdates.role;

        // Update user (permission checked by middleware)
        const updatedUser = await updateUser(validatedId.id, updateData);

        logger.info(`User ${validatedId.id} updated successfully by user ${req.user?.id}`);
        res.json({
...
export const deleteUserData = async (req, res, next) => {
    try{
        logger.info(`Deleting user with ID: ${req.params.id} by admin user ${req.user?.id}`);
        ...
        logger.info(`User ${validatedId.id} deleted successfully by admin ${req.user?.id}`);
        res.json({
